...
public class RePair {
	...
	public byte[] encode(byte[] data) throws UnsupportedEncodingException {
		...
		log.debug("Komprimováno s nahrazením " + dict.size() + " znakù");
		byte[] d = concate(dict, after, afterLength);
		log.debug("Komprimace ukonèena (" + (d.length / (double) data.length)
				* 100 + " %)");
		return d;
	}
	...
	private byte[] concate(List<Symbol> symbols, byte[] textBytes,
			int byteLength) {
	...
	private void replace(byte[] init, int initLength, Symbol symbol,
			int symbolCount, byte replaceWith) {
		...
		for (int i = (afterLength - newArrayIndex); i > 0; i--)
			after[newArrayIndex++] = init[initLength
					- (afterLength - newArrayIndex) - 1];
	}
	...
	public byte[] decode(byte[] data) throws UnsupportedEncodingException {
		...
		log.debug("Dekomprimace ukonèena");
		return data;
	}
}
